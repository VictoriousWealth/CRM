...
public class CustomJwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (Arrays.stream(SecurityConfig.openEndUrls).toList().contains(request.getServletPath())) {
        ...
        try {
            ...
            CustomJwtAuthenticationToken authentication = new CustomJwtAuthenticationToken(token);
            Authentication authResult = authenticationManager.authenticate(authentication);
            System.out.println("Authentication Manager has authenticated: <" + authResult +">.");
            SecurityContextHolder.getContext().setAuthentication(authResult);
