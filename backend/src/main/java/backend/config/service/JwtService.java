...
public class JwtService {
    ...
    public String[] validateTokenAndGetUsernameAndRole(String token) throws Exception {
        ...
        boolean issuerNotTrusted = isIssuerNotTrusted(decodedPayload);
        System.out.println(issuerNotTrusted ? "issuer is not trusted" : "issuer is trusted");
        if (issuerNotTrusted) return new String[2];
        boolean tokenExpired = isTokenExpired(decodedPayload);
        System.out.println(tokenExpired ? "token is expired" : "token is not expired");
        if (tokenExpired) return new String[2];


        String[] strings = extractUsernameAndRole(decodedPayload);
        System.out.println("Username: "+strings[0] + " Role: "+strings[1]);
        return strings;
    }

    private String[] extractUsernameAndRole(String decodedPayload) throws Exception {
        System.out.println("extractUsernameAndRole has been called");
    ...
    private boolean isIssuerNotTrusted(String decodedPayload) throws Exception {
        ...
        System.out.println("issuer: "+issuer +
                (!issuer.equals("Backend")?" hence it is not trusted":" hence it is trusted"));

        return !issuer.equals("Backend");
    }
    ...
    private boolean isTokenExpired(String decodedPayload) throws Exception {
        ...
        System.out.println(issueTime + " ? "+timeInSeconds+" ? "+expiryTime + " thus " + isTokenExpired);
    ...
    public String generateToken(String username, String role) throws NoSuchAlgorithmException, InvalidKeyException {
        ...
        String payload = "{\n" +
                ...
                "  \"sub\": \""+username+"\",\n" +
                "  \"role\": \""+role+"\",\n" +
                "  \"iat\": "+ (timeInSeconds-100) +",\n" +
                "  \"exp\": "+(timeInSeconds+3700)+"\n" +
                "}";
